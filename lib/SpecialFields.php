...
class SpecialFields {
	...
	public static function getDomain( $fields ) {
		return static::getExtrafields( $fields, self::$domainFields );
	}

	public static function getContact( $fields ) {
		return static::getExtrafields( $fields, self::$domainFields );
	}

	private static function getExtrafields( $fields, $ref ) {
		$result = [];
		foreach ( $fields as $key => $value ) {
			if ( in_array( $key, $ref ) && is_scalar( $value ) ) {
				$result[ $key ] = $value;
			}
		}
